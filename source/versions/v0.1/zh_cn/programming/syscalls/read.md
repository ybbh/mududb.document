...
- `options.for_update` —— 为 true 时，读取会获取锁，以允许在同一事务内安全地进行后续更新。

## 行为

- 在表或索引中执行点查找，返回匹配的行；如果未找到则返回 null。
...
- `NotFound`：不是错误；未匹配时 `read` 返回 `null`。
- `LockConflict`：请求 `for_update` 且锁获取失败时返回。

## 事务语义

- 在打开的事务内进行的读取受该事务的快照/隔离影响。
- 事务外的读取遵循宿主默认语义。
